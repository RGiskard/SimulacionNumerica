...
int main()
{
	...
	TPZVec<std::string> scalarVars(2), vectorVars(0);
	scalarVars[0] = "Solution";
	scalarVars[1] = "POrder";
	an.DefineGraphMesh(2, scalarVars, vectorVars, "solucao.vtk");
	int resolution = 0;
	an.PostProcess(resolution);
	REAL Tolerance = pow(10, -5);
	TPZManVector<int64_t> elfathers, elsons;
	//IdentifyingElementsByEthakBig(an, Tolerance, elfathers);
	//UniformHRefinementSelection(1, mallaGeo, elfathers);
	//ofstream salidaVTKREFIN("mallaGeometricaFunREfin.vtk");
	//TPZVTKGeoMesh::PrintGMeshVTK(mallaGeo, salidaVTKREFIN, true);
